...
ptsd.data.filters.uninterestingTools = function(toolName){
    var retVal = true
    var uninterestingNames = [
    "home",
    "rid",
    "manage",
    "assess",
    ...
    "assessmentDailyOnly",
    "assessmentTooSoon",
    "assessmentDailyAndWeekly"
    ]
    $(uninterestingNames).each(function(){
        if(toolName == this.toString()){
            retVal = false;
        }
    })
    if(toolName[0] === "@" || toolName === "NONE"){
        retVal = false;
    }
    return retVal
}
...
ptsd.data.filters.toolParticipation = function(data){
    if(data.survey_description){
        if(data.survey_id === "contentScreenViewedProbe" ){
            var response = data.responses.contentScreenName.prompt_response 
            if(ptsd.data.filters.uninterestingTools(response)){
                var datum = {}
